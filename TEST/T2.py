...
- Multi-page pager (PDF & multi-frame TIFF) — floating rounded chip at bottom-center
...
class FileViewerApp:
    ...
    def _build_layout(self):
        ...
        # Outer padding frame (visual margin around whole app)
        ...
        # =============== OUTER LEFT | RIGHT =================
        self.pw = tk.PanedWindow(outer, orient=tk.HORIZONTAL, sashrelief=tk.RAISED)
        ...
        topbar = ttk.Frame(self.right_top)
        ...
        self.canvas = tk.Canvas(self.preview_border, height=200, highlightthickness=0, bg="#303030")
        ...
        # Row: Previous / Next
        nav_row = ttk.Frame(self.right_top)
        ...
        # Row: Fit/Zoom/Help
        zoom_row = ttk.Frame(self.right_top)
        ...
        # Row: Tools
        tools_row = ttk.Frame(self.right_top)
        ...
        # Start hidden until a multi-page doc is opened
        self.page_overlay.place_forget()
        self.page_overlay_visible = False

        # Reposition overlay when preview resizes
        self.preview_border.bind("<Configure>", lambda e: self._position_pager() if self.page_overlay_visible else None)
    ...
    # --- pager overlay methods ---
    def _chip_bg_color(self):
        # Subtle neutral background; not pure white to avoid visible fringe
        return "#f5f6f8"

    def _build_pager_overlay(self):
        # Canvas that draws a rounded background (and soft shadow), with a frame on top
        self.page_overlay = tk.Canvas(
            self.preview_border,
            highlightthickness=0,
            bd=0,
            bg=self._chip_bg_color()
        )
        self.page_overlay_visible = False  # track visibility

        # Inner frame to hold controls; use tk.Frame so bg can match chip
        self.page_bar = tk.Frame(self.page_overlay, bg=self._chip_bg_color(), bd=0, highlightthickness=0)
        ...
        # Layout inside chip
        self.page_prev_btn.grid(row=0, column=0, padx=(6, 4), pady=6)
        tk.Label(self.page_bar, text="Page", bg=self._chip_bg_color()).grid(row=0, column=1, padx=(0, 4), pady=6)
        self.page_entry.grid(row=0, column=2, padx=(0, 4), pady=6)
        self.page_label.grid(row=0, column=3, padx=(4, 8), pady=6)
        self.page_go_btn.grid(row=0, column=4, padx=(0, 8), pady=6)
        self.page_next_btn.grid(row=0, column=5, padx=(0, 6), pady=6)

        # Embed frame into canvas
        self._pager_window_id = self.page_overlay.create_window(0, 0, window=self.page_bar, anchor="nw")

    def _position_pager(self):
        # Size the chip to its contents, draw rounded bg, and place at bottom-center
    ...
    def _bind_events(self):
        ...
        # Up/Down navigate files (your request)
        ...
        # pager entry
        # (bound after build, still works when overlay is visible)
        self.root.bind_all("<Return>", lambda e: self.page_go() if self.page_overlay_visible and self.page_entry.focus_get() == self.page_entry else None)
    ...
    def _set_page_nav_visible(self, visible: bool):
        # The overlay is only shown for multi-page docs
        if visible:
    ...
    def open_path(self, path: str):
        ...
        # Default zoom: Fit Width for new image/PDF
        if self.mode == 'image':
            self.fit_width()
        self._update_status()
