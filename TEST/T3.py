...
class LandRecordOrganizer:
    def __init__(self, root):
        ...
        # Add event to update entry fields when a record is selected
    ...
    def on_treeview_select(self, event):
        """Update entry fields when a record is selected in the Treeview"""
        selected = self.tree.selection()
        if selected:
            uuid = self.tree.item(selected[0], "values")[0]
            record = self.records.get(uuid, {})
            self.entry_name.delete(0, tk.END)
            self.entry_comments.delete(0, tk.END)

            # Populate entry fields with the selected record's data
            if record:
                self.entry_name.insert(0, record.get("name", ""))
                self.entry_comments.insert(0, record.get("comments", ""))
    ...
    def add_record(self):
        """Add a new record to the Treeview with proper hierarchy"""
        selected = self.tree.selection()
