...
class BulkRenamer:
    ...
    def apply_changes(self):
        ...
        # Apply renaming logic to the files that are currently selected or loaded
    ...
    def apply_rename_logic(self):
        ...
        for i, (file_path, original_name) in enumerate(all_files):
            ...
            # Apply or remove the prefix and suffix based on the checkbox state
            if self.custom_attributes.get():
                ...
                # Only add prefix if it isn't already present
                if not name.startswith(prefix):
                    name = prefix + name
                
                # Only add suffix if it isn't already present
                if not name.endswith(suffix):
                    name = name + suffix

                proposed_name = name + ext
            else:
                # Remove the prefix and suffix if the checkbox is unchecked
                prefix = self.prefix.get() if self.prefix.get() else ""
                suffix = self.suffix.get() if self.suffix.get() else ""
                
                # Check if the name starts with the prefix and ends with the suffix, then remove them
